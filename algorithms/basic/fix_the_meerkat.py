"""
You're at the zoo... all the meerkats look weird. Something has gone terribly wrong - someone has gone and switched 
their heads and tails around!

Save the animals by switching them back. You will be given an array which will have three values (tail, body, head). 
It is your job to re-arrange the array so that the animal is the right way round (head, body, tail).

Same goes for all the other arrays/lists that you will get in the tests: you have to change the element positions with the 
same exact logics

URL: https://www.codewars.com/kata/56f699cd9400f5b7d8000b55
"""





def fix_the_meerkat(arr: list[str]) -> list[str]:
    """
    Dada una lista con tres elementos en orden invertido (cola, cuerpo, cabeza),
    retorna la lista en el orden correcto (cabeza, cuerpo, cola).

    Args:
        arr (list[str]): Lista con exactamente tres cadenas representando partes del cuerpo.

    Returns:
        list[str]: Lista con los elementos en el orden correcto.
    """
    return arr[::-1]


print(fix_the_meerkat(["tail", "body", "head"]))  # ['head', 'body', 'tail']
print(fix_the_meerkat(["bottom", "middle", "top"]))  # ['top', 'middle', 'bottom']
print(fix_the_meerkat(["a", "b", "c"]))  # ['c', 'b', 'a']
